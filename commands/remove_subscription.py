...
async def remove_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with connection_pool.connection() as connection:
        ...
        status, res = await sub_repo.remove(Subscription(id=None, user_id=user.id, title=title))
        if not status:
            logging.error(res)
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Не удалось удалить подписку, попробуйте позже."
            )
        return

    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"Подписка на «{title}» успешно удалена!"
    )